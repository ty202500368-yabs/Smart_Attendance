...
/* ── BSCS Prospectus version (New / Old) ── */
function setBSCSVersion(ver) {
  // panels
  document.getElementById('panel-bscs-new').style.display = ver === 'new' ? '' : 'none';
  document.getElementById('panel-bscs-old').style.display = ver === 'old' ? '' : 'none';
  // buttons
  document.getElementById('btn-bscs-new').classList.toggle('active', ver === 'new');
  document.getElementById('btn-bscs-old').classList.toggle('active', ver === 'old');
  // subtitle
  const sub = document.getElementById('bscs-subtitle');
  if (sub) sub.textContent = ver === 'new'
    ? 'BSCS 2023–2024 New Curriculum · Ladderized with 2-yr ACT'
    : 'BSCS 2018–2019 Old Curriculum · CMO No. 25, s. 2015';
}

/* ── BSIT Prospectus version (New / Old) ── */
function setBSITVersion(ver) {
  document.getElementById('panel-bsit-new').style.display = ver === 'new' ? '' : 'none';
  document.getElementById('panel-bsit-old').style.display = ver === 'old' ? '' : 'none';
  document.getElementById('btn-bsit-new').classList.toggle('active', ver === 'new');
  document.getElementById('btn-bsit-old').classList.toggle('active', ver === 'old');
  const sub = document.getElementById('bsit-subtitle');
  if (sub) sub.textContent = ver === 'new'
    ? 'BSIT 2023–2024 New Curriculum'
    : 'BSIT Old Curriculum';
}

/* ── Legacy functions kept for compatibility ── */
function setProspectusProgram(el, prog) {
  // redirect to correct dedicated page
  if (prog === 'bscs') { gotoPage('pros-bscs'); }
  else if (prog === 'bsit') { gotoPage('pros-bsit'); }
}
function setProspectusVersion(ver) {
  // used on main prospectus page (now just redirects)
}
