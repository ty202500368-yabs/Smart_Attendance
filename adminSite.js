...
/* ── Subject Verification dept tabs ── */
function switchSVDept(el, panelId) {
  el.closest('.card').querySelectorAll('.dept-tab').forEach(t => t.classList.remove('active'));
  el.classList.add('active');
  document.querySelectorAll('#page-students-subject-verify .sv-dept-panel').forEach(p => p.style.display = 'none');
  document.getElementById(panelId).style.display = 'block';
}

/* ── Subject Verification student search ── */
function filterSVStudents() {
  const q = document.getElementById('sv-search')?.value.toLowerCase() || '';
  document.querySelectorAll('.sv-student-row').forEach(r => {
...
/* ── Subject Verification detail modal ── */
let _svCurrentSubjects = [];
let _svCurrentCor = null;
let _svCorVisible = false;

function openSVModal(data) {
  _svCurrentSubjects = data.subjects || [];
  _svCurrentCor = data.corFile || null;
  _svCorVisible = false;

  // Hide COR panel on open
  const corPanel = document.getElementById('sv-cor-panel');
  corPanel.style.display = 'none';
  const toggleBtn = document.getElementById('sv-cor-toggle-btn');
  ...
  const colors = ['#7b1728','#3d5a2e','#5b3a7e','#1b5e8c','#c96a10'];
  const colorIdx = data.name.charCodeAt(0) % colors.length;
  document.getElementById('sv-avatar').textContent = initials;
  document.getElementById('sv-avatar').style.background = colors[colorIdx];
  document.getElementById('sv-detail-name').textContent = data.name;
  document.getElementById('sv-detail-meta').textContent = `${data.id} · ${data.section} · ${data.program} · ${data.email}`;

  const statusBadge = document.getElementById('sv-detail-status-badge');
  if (data.subjectStatus === 'verified') {
    statusBadge.innerHTML = '<span class="badge badge-green"><svg width="8" height="8" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:middle"><circle cx="12" cy="12" r="10"/></svg> Verified</span>';
  } else if (data.subjectStatus === 'rejected') {
    statusBadge.innerHTML = '<span class="badge badge-red">Rejected</span>';
  } else {
    statusBadge.innerHTML = '<span class="badge badge-orange">Pending</span>';
  }

  document.getElementById('sv-modal-title').innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/></svg> Subject Verification — ${data.name}`;

  // Populate COR panel info
  if (_svCurrentCor) {
    const cor = _svCurrentCor;
    document.getElementById('sv-cor-filename').textContent = cor.name;
    document.getElementById('sv-cor-filesize').textContent = `· ${cor.size} · PDF`;
    document.getElementById('sv-cor-preview-name').textContent = data.name + ' · ' + data.id;

    const chip = document.getElementById('sv-cor-status-chip');
    const statusMap = {
      verified:  { label:'VERIFIED',  bg:'var(--green)',  color:'#fff' },
      mismatch:  { label:'MISMATCH',  bg:'var(--orange)', color:'#fff' },
      new:       { label:'NEW',       bg:'var(--maroon)', color:'#fff' },
      rejected:  { label:'REJECTED',  bg:'var(--red)',    color:'#fff' },
    };
    const s = statusMap[cor.corStatus] || statusMap.new;
    chip.textContent = s.label;
    chip.style.background = s.bg;
    chip.style.color = s.color;

    // COR subject cross-reference list
    const corSubList = document.getElementById('sv-cor-subject-list');
    corSubList.innerHTML = `
      <div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.8px;margin-bottom:4px;">Subjects on COR</div>
      ${_svCurrentSubjects.map(s => {
        const icon = s.status === 'verified'
          ? `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>`
          : s.status === 'rejected'
          ? `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`
          : `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--orange)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`;
        return `<div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;border-radius:6px;background:#fff;border:1px solid var(--border);font-size:12px;">
          <span style="font-weight:600;">${s.code}</span>
          <div style="display:flex;align-items:center;gap:4px;">${icon}<span style="font-size:11px;color:var(--muted);">${s.units}u</span></div>
        </div>`;
      }).join('')}
      <div style="margin-top:6px;font-size:10.5px;color:var(--muted);text-align:center;line-height:1.5;">Submitted: ${cor.submitted}</div>
    `;
  }

  filterSVModal('all');
  openModal('modal-sv-detail');
}

function toggleSVCor() {
  _svCorVisible = !_svCorVisible;
  const corPanel = document.getElementById('sv-cor-panel');
  const btn = document.getElementById('sv-cor-toggle-btn');
  if (_svCorVisible) {
    corPanel.style.display = 'flex';
    btn.style.background = 'var(--maroon)';
    btn.style.color = '#fff';
    btn.style.borderColor = 'var(--maroon)';
    btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Hide COR`;
  } else {
    corPanel.style.display = 'none';
    btn.style.background = '#fff';
    btn.style.color = 'var(--text)';
    btn.style.borderColor = 'var(--border)';
    btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> View COR`;
  }
}

function filterSVModal(status) {
  const tabColors = { all:'var(--maroon)', pending:'var(--orange)', verified:'var(--green)', rejected:'var(--red)' };
  ['all','pending','verified','rejected'].forEach(t => {
    const btn = document.getElementById('svmod-tab-' + t);
    if (!btn) return;
    const isActive = t === status;
    btn.style.color = isActive ? tabColors[t] : 'var(--muted)';
    btn.style.borderBottom = isActive ? `2.5px solid ${tabColors[t]}` : '2.5px solid transparent';
  });

  const list = document.getElementById('sv-subject-list');
  const filtered = status === 'all' ? _svCurrentSubjects : _svCurrentSubjects.filter(s => s.status === status);

  if (filtered.length === 0) {
    list.innerHTML = `<div style="padding:40px;text-align:center;color:var(--muted);font-size:13px;">
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="display:block;margin:0 auto 10px;opacity:.3"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/></svg>
      No ${status === 'all' ? '' : status + ' '}subjects found.
    </div>`;
    return;
  }

  list.innerHTML = filtered.map(s => {
    const statusColor = s.status === 'verified' ? 'var(--green)' : s.status === 'rejected' ? 'var(--red)' : 'var(--orange)';
    const statusLabel = s.status === 'verified' ? 'Verified' : s.status === 'rejected' ? 'Rejected' : 'Pending';
    ...
    const noteHtml = s.note ? `<div style="font-size:11px;color:var(--orange);font-weight:600;margin-top:3px;display:flex;align-items:center;gap:4px;"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>${s.note}</div>` : '';
    const actions = s.status === 'pending' ? `
      <div style="display:flex;gap:6px;flex-shrink:0;flex-wrap:wrap;">
        <button class="btn btn-success btn-sm" onclick="svApproveSubject(this,'${s.code}')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg> Approve
        </button>
        <button class="btn btn-danger btn-sm" onclick="svRejectSubject(this,'${s.code}')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Reject
        </button>
      </div>` : '';
    return `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 16px;border:1.5px solid var(--border);border-radius:var(--r-sm);background:#faf8f5;flex-wrap:wrap;transition:border-color .2s;" onmouseover="this.style.borderColor='${statusColor}'" onmouseout="this.style.borderColor='var(--border)'">
        <div style="display:flex;align-items:center;gap:12px;flex:1;min-width:0;">
          <div style="width:34px;height:34px;border-radius:8px;background:${statusColor}18;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:${statusColor};">${statusIcon}</div>
          <div>
            <div style="font-weight:700;font-size:13.5px;">${s.code}</div>
            <div style="font-size:11.5px;color:var(--muted);">${s.units} units</div>
            ${noteHtml}
          </div>
        </div>
        <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
          <span style="font-size:11px;font-weight:700;padding:3px 10px;border-radius:20px;background:${statusColor}18;color:${statusColor};border:1px solid ${statusColor}40;">${statusLabel}</span>
          ${actions}
        </div>
      </div>`;
  }).join('');
...
function svApproveSubject(btn, code) {
  const subj = _svCurrentSubjects.find(s => s.code === code);
  if (subj) { subj.status = 'verified'; delete subj.note; }
  _refreshSVCorCrossRef();
  filterSVModal(document.querySelector('[id^="svmod-tab-"][style*="2.5px solid var"]')?.id?.replace('svmod-tab-','') || 'all');
}

function svRejectSubject(btn, code) {
  const subj = _svCurrentSubjects.find(s => s.code === code);
  if (subj) { subj.status = 'rejected'; }
  _refreshSVCorCrossRef();
  filterSVModal(document.querySelector('[id^="svmod-tab-"][style*="2.5px solid var"]')?.id?.replace('svmod-tab-','') || 'all');
}

function _refreshSVCorCrossRef() {
  const corSubList = document.getElementById('sv-cor-subject-list');
  if (!corSubList || !_svCurrentCor) return;
  const rows = _svCurrentSubjects.map(s => {
    const icon = s.status === 'verified'
      ? `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--green)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"/></svg>`
      : s.status === 'rejected'
      ? `<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--red)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`
  ...
  const existing = corSubList.innerHTML;
  const labelEnd = existing.indexOf('</div>') + 6;
  corSubList.innerHTML = existing.substring(0, labelEnd) + rows + existing.substring(existing.lastIndexOf('<div style="margin-top:6px'));
}

function saveAndCloseSVModal() {
  closeModal('modal-sv-detail');
  alert('Subject verification changes saved successfully!');
}
